...
public class Program {
	...
	DESA.connectDatabase(Database.PROTOCOL_JDBC, Database.VENDOR_ORACLE,
		Database.DRIVER_THIN, Database.DEFAULT_SERVER,
		Database.DEFAULT_PORT, Database.DEFAULT_DATABASE,
		Database.DESA_USER, Database.DESA_PASS);
	...
	// exercise6_1();
	// exercise7_1();
	exercise6_1Function(1);
	exercise7_1Procedure("barcelona");
     ...
     * Pr�ctica 2. 1.1. 21. Obtener el nombre y el apellido de los clientes
     * que han adquirido un coche en un concesionario de Madrid, el cual dispone
     * de coches del modelo gti.
     */
    public static void exercise1_1() throws SQLException {
	String query = ("SELECT DISTINCT clientes.nombre, clientes.apellido "
	...
	while (rs.next()) {
	    ...
	    System.out.println(cif
		    + " " + nombre + " " + ciudad + " " + cantidad + " "); //$NON-NLS-2$ //$NON-NLS-3$ //$NON-NLS-4$
	}
    }

    /*
     * 2. Mostrar por pantalla el resultado de la consulta 6 de la Pr�ctica 2
     * de forma el color de la b�squeda sea introducido por el usuario. 6.
     * Obtener el nombre de las marcas de las que se han vendido coches de un
     * color introducido por el usuario.
     */
	...
	String sql = "SELECT  clientes.nombre FROM clientes "
		...
		+ "WHERE clientes.dni=ventas.dni "
		+ "AND ventas.cifc=concesionarios.cifc "
		+ "AND concesionarios.ciudadc = ?" + "AND ventas.color = ?)";
    ...
    /*
     * 5. Realizar una aplicaci�n en Java que haciendo uso de la instrucci�n
     * SQL adecuada: 5.1. Introduzca datos en la tabla coches cuyos datos son
	...
	try {
	    ...
	    System.out.println("REGISTER CREATED: " + parameters[0] + ", "
		    + parameters[1] + ", " + parameters[2]);
	} catch (SQLException e) {
	    System.err.println("ERROR: register " + parameters[0] + ", "
		    + parameters[1] + ", " + parameters[2]
		    + "couldn't be deleted.");
	    e.printStackTrace();
	}
	...
	String sql = "UPDATE coches " + "SET nombrech = ?, modelo = ? "
		+ "WHERE codcoche = ?";
    ...
    /*
     * 6. Invocar la funci�n y el procedimiento del ejercicio 10 de la
     * Pr�ctica 10 desde una aplicaci�n Java. 10. Realizar un procedimiento
     * y una funci�n que dado un c�digo de concesionario devuelve el
     * n�mero ventas que se han realizado en el mismo. 6.1. Funcion
     */
    public static void exercise6_1() throws SQLException {
	System.out
		.print("Enter the cif code of the dealer you want to consult: ");
	int cifc = ReadInt();
	exercise6_2(cifc);
    }
     ...
     * FUNCTION...
     * 
     * @param cifc is the cif of the dealer.
     * @throws SQLException
     */
    public static void exercise6_1Function(int cifc) throws SQLException {
	String statement = "{? = call FUNCTION10(?)}";
	Integer[] parameters = new Integer[1];
	parameters[0] = cifc;
	Integer[] types = new Integer[1];
	types[0] = java.sql.Types.INTEGER;
	
	for(Object o : DESA.callFunction(statement, types, parameters)) {
     ...
     * @throws SQLException
     */
    public static void exercise6_1Procedure(int cifc) throws SQLException {
	String statement = "{call PROCEDURE10(?)}";
	Integer[] parameters = new Integer[1];
	parameters[0] = cifc;
	Integer[] types = new Integer[1];
	types[0] = java.sql.Types.INTEGER;
	
	for(Object o : DESA.callStoredProcedure(statement, types, parameters)) {
	    System.out.println(o.toString());
	}
	...
	String sql = "SELECT COUNT(cifc) " + "FROM ventas "
		+ "WHERE ventas.cifc = ?";
	Integer[] parameters = new Integer[1];
	parameters[0] = cifc;
	rs = DESA.executeSQL(sql, parameters);
	...
	while (rs.next()) {
	    System.out.println("The dealer with cif " + cifc + " has sold "
		    + rs.getString(1) + " cars.");
	}
    }

    /*
     * 7. Invocar la funci�n y el procedimiento del ejercicio 11 de la
     * Pr�ctica 10 desde una aplicaci�n Java. 11. Realizar un procedimiento
     * y una funci�n que dada una ciudad que se le pasa como par�metro
     * devuelve el n�mero de clientes de dicha ciudad. 7.1. Funcion
     */
     ...
     * FUNCTION...
     * 
     * @param city to consult.
     * @throws SQLException
     */
    public static void exercise7_1Function(String city) throws SQLException {
	String statement = "{? = call FUNCTION11(?)}";
	String[] parameters = new String[1];
	parameters[0] = city;
	Integer[] types = new Integer[1];
	types[0] = java.sql.Types.INTEGER;
     ...
     * @throws SQLException
     */
    public static void exercise7_1Procedure(String city) throws SQLException {
	String statement = "{call PROCEDURE11(?)}";
	String[] parameters = new String[1];
	parameters[0] = city;
	Integer[] types = new Integer[1];
	types[0] = java.sql.Types.INTEGER;
	...
	String sql = "SELECT COUNT(dni) " + "FROM clientes "
	...
	while (rs.next()) {
	    System.out.println("The city " + city + " has " + rs.getString(1)
		    + " clients.");
	}
    }
	...
	while (rs.next()) {
	    System.out.println(" REGISTER: " + rs.getString(1) + ", "
